...
<thead>
	<tr>
		...
		<th width="20%" class="text-center">자릿수</th>
...
<%
	...
	ArrayList<Theater> list = theater.getList(MOVIETHEATER_NO);
	for(int i=0;i<list.size();i++){
		int row = list.get(i).getTHEATER_ROW();
		int column = list.get(i).getTHEATER_COLUMN();
		int size = row * column;
		String theatername = list.get(i).getTHEATER_NAME() + "관";
%>
	<tr>
		<th width="10%" class="text-center"><%=i+1%></th>
		<th width="20%" class="text-center"><%=theatername %></th>
		...
		<form method="post" action="TheaterDelete.jsp?THEATER_NO=<%=list.get(i).getTHEATER_NO() %>" onsubmit="return check()">
