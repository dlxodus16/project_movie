...
<thead>
	<tr>
		...
		<th width="20%" class="text-center">관람객 수</th>
		<th width="10%" class="text-center">평점</th>
		<th width="20%" class="text-center">총금액</th>
...
<%
	...
	for(int i=0;i<list.size();i++){
		...
		float avg = review.getAvg(list.get(i).getMOVIE_NO()).getREVIEW_AVG();
		String num = String.format("%.1f",avg);
		DecimalFormat format = new DecimalFormat("###,###");
		int money = movie.getMoney(list.get(i).getMOVIE_NO());
		String money_string = format.format(money);
%>
	<tr>
		<th width="10%" class="text-center"><%=i+1%></th>
		<th width="30%" class="text-left"><a href="viewMovie.jsp?MOVIE_NO=<%=list.get(i).getMOVIE_NO()%>"><%=list.get(i).getMOVIE_TITLE() %></a></th>
		<th width="20%" class="text-center"><%=list.get(i).getMOVIE_VISITOR() %></th>
		<th width="10%" class="text-center"><%=num %></th>
		<th width="20%" class="text-center"><%=money_string %></th>
		<form method="post" action="MovieUpdate.jsp?MOVIE_NO=<%=list.get(i).getMOVIE_NO() %>">
		<th width="10%" class="text-center"><input type="submit" value="수정"></th></form>
		<form method="post" action="MovieDelete.jsp?MOVIE_NO=<%=list.get(i).getMOVIE_NO() %>" onsubmit="return check()">
