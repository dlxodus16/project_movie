...
<thead>
	<tr>
		...
		<th width="20%" class="text-center">영화제목</th>
		<th width="20%" class="text-center">영화관이름</th>
		<th width="20%" class="text-center">상영날짜</th>
...
<%
	...
	ArrayList<Showing> list = showing.getShowingList();
	for(int i=0;i<list.size();i++){
		String movie_title = showing.getMovieTitle(list.get(i).getMOVIE_NO());
		String movietheater_name = showing.getMovieTheaterName(list.get(i).getMOVIETHEATER_NO());
		String showing_date = list.get(i).getSHOWING_DATE();
		showing_date = showing_date.substring(0,10);
%>
	<tr>
		<th width="10%" class="text-center"><a href="ShowingDetailManagement.jsp?SHOWING_NO=<%=list.get(i).getSHOWING_NO()%>"><%=i+1%></a></th>
		<th width="20%" class="text-center"><%=movie_title %></th>
		<th width="20%" class="text-center"><%=movietheater_name %></th>
		<th width="20%" class="text-center"><%=showing_date %></th>
	</tr>
		<%
		}
		%>
